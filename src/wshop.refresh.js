...
var wshopRefresh = {
    ...
    processAllProducts: function(products) {
        ...
        // Clear message
        jQuery('.refresh-message').html(
            '<li>Received ' +
                products.length +
                ' product(s) from Shopify...</li>'
        )
    ...
    processNextProduct: function(index) {
        ...
        jQuery
            ...
            .done(function(message) {
                ...
                jQuery('.refresh-message').prepend(
                    '<li>(' +
                        wshopRefresh.processedIDs.length +
                        ' / ' +
                        wshopRefresh.totalProducts +
                        ') ' +
                        message.message +
                        '</li>'
                )
    ...
    removeOldProducts: function() {
        ...
        jQuery
            ...
            .done(function(message) {
                // Append message
                jQuery('.refresh-message').prepend(
                    '<li>Cleaning up products removed from Shopify...</li>'
                )
                ...
                if (!extraProductPages.length) {
                    ...
                    jQuery('.refresh-message').prepend(
                        '<li>No old products to clean up.</li>'
                    )
    ...
    processAllCollections: function() {
        ...
        jQuery('.refresh-message').prepend(
            '<li>Received ' +
                wshopRefresh.collections.length +
                ' collection(s) from Shopify...</li>'
        )
    ...
    processNextCollection: function() {
        ...
        var collection = {
            ...
            image: nextCollection.attrs.image
                ? nextCollection.attrs.image.src
                : false
        }
        ...
        jQuery
            ...
            .done(function(message) {
                ...
                wshopRefresh.shopClient
                    .fetchQueryProducts({
                        limit: 1000,
                        collection_id: nextCollection.attrs.collection_id
                    })
                    .then(result => {
